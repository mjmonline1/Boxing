...
* **`CreateDummyDataFile.js`**: Transforms raw data (from PDF or CSV) into the standardized format required by the filtering engine.
* **`PutAllFightersinbuckets.js`**: A specialized version of the tournament script focused on outputting bucket structures rather than individual boxer details.
...
* **`RegisteredBoxers2025.csv`**: The master roster (137 boxers) including club, gender, YOB, weight, and experience.
